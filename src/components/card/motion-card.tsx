...
import type { PoliticianMotion } from "@prisma/client";
import { placeholderBlurhash, toDateString } from "@/lib/utils";
import { Card, Link } from "@nextui-org/react";

interface MotionCardProps {
  data: PoliticianMotion,
}

export default function MotionCard({ data }: MotionCardProps) {
  return (
    <Card
      as={Link}
      isPressable
      href={`mocoes/${data.slug}`}
      className="ease rounded-2xl border-2 border-stone-100 bg-white shadow-md transition-all duration-200 hover:-translate-y-1 hover:shadow-xl dark:border-stone-800 xl:flex"
    >
      <BlurImage
        src={data.image!}
